...
export async function generateMetadata() {
  ...
  const strings = await getAutoTranslatedStrings(
    locale,
    getRegionalActivationStrings("de"),
    getRegionalActivationStrings(locale),
  );
  return strings.meta;
}
        ...
        <div className="mx-auto max-w-6xl px-5 py-20 md:px-8 md:py-28">
          ...
          <div className="mt-9 grid gap-4 md:grid-cols-3">
            {strings.page.steps.map((step) => (
              <article key={step.title} className="rounded-3xl border border-[#f4f1e8]/10 bg-[#0b1714] p-5">
                <h2 className="font-black text-[#d6ff65]">{step.title}</h2>
                <p className="mt-2 text-sm leading-6 text-[#f4f1e8]/55">{step.body}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-5 py-16 md:px-8 lg:grid-cols-[0.82fr_1.18fr] lg:py-24">
        <div className="space-y-5">
          <article className="rounded-3xl border border-[#f4f1e8]/10 bg-[#0b1714] p-6">
            <h2 className="text-2xl font-black tracking-[-0.03em]">{strings.page.todayTitle}</h2>
            <p className="mt-3 leading-7 text-[#f4f1e8]/58">{strings.page.todayBody}</p>
          </article>
          <article className="rounded-3xl border border-[#f4f1e8]/10 bg-[#0b1714] p-6">
            <h2 className="text-2xl font-black tracking-[-0.03em]">{strings.page.laterTitle}</h2>
            <p className="mt-3 leading-7 text-[#f4f1e8]/58">{strings.page.laterBody}</p>
          </article>
          <article className="rounded-3xl border border-[#d6ff65]/22 bg-[#d6ff65]/7 p-6">
            <h2 className="text-2xl font-black tracking-[-0.03em]">{strings.page.responsibilityTitle}</h2>
            <p className="mt-3 leading-7 text-[#f4f1e8]/62">{strings.page.responsibilityBody}</p>
          </article>
          <Link
