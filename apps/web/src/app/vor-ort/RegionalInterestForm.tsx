...
export default function RegionalInterestForm({
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (intents.length === 0) {
      setNotice({ ok: false, message: strings.form.notices.intentionRequired });
      return;
    }
    if (!contactConsent || !privacyAccepted) {
      setNotice({ ok: false, message: strings.form.notices.consentRequired });
      return;
    }
    if (!humanToken) {
    ...
    try {
      ...
      if (!response.ok || !data?.ok) throw new Error("regional_interest_failed");
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {strings.form.intentionOptions.map((option) => (
              ...
              >
                ...
                />
                <span>
                  <span className="block font-bold text-[#f4f1e8]">{option.label}</span>
                  <span className="mt-1 block text-xs leading-5 text-[#f4f1e8]/45">{option.hint}</span>
                </span>
              </label>
            ))}
          </div>
        ...
        <div className="space-y-3 rounded-2xl border border-[#f4f1e8]/10 bg-[#07110f]/65 p-4">
          ...
          <label className="flex items-start gap-3 text-sm leading-6 text-[#f4f1e8]/62">
            ...
            <span>
              ...
              <Link href="/datenschutz" className="font-bold text-[#d6ff65] underline underline-offset-4">
